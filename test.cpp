...
void dis2(int x1, int y1, int x2, int y2)
{
    vector<int> tmp(n);
    if (y1 < y2)
        dis(x1, y1, x2, y2, max(y1 - 2, 0), min(y2 + 2, 5 * n - 1));
    else
        dis(x1, y1, x2, y2, max(y2 - 2, 0), min(y1 + 2, 5 * n - 1));
    if (y1 < y2)
    {
        for (int i = 0; i < n; i++)
            tmp[(y1 + i) % n] = vis[0][i + y1];
    }
    else
    {
        for (int i = 0; i < n; i++)
            tmp[(y1 - i + n) % n] = vis[0][y1 - i];
    }
    for (int i = 0; i < n; i++)
        tmp[i] += vis[x2][y2];
    if (y1 < y2)
        dis(x2, y2, x1, y1, max(y1 - 2, 0), min(y2 + 2, 5 * n - 1));
    else
        dis(x2, y2, x1, y1, max(y2 - 2, 0), min(y1 + 2, 5 * n - 1));
    if (y2 < y1)
    {
        for (int i = 0; i < n; i++)
            tmp[(y2 + i) % n] += vis[n - 1][i + y2];
    }
    else
    {
        for (int i = 0; i < n; i++)
            tmp[(y2 - i + n) % n] += vis[n - 1][y2 - i];
    }
    for (int i = 0; i < n; i++)
